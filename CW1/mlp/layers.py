...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if (stochastic == True):
            mask_size = inputs.shape[1:]
            if (self.share_across_batch):
                mask_size = inputs.shape[1:]
                self.u = self.rng.uniform(0,1, size = mask_size) < self.incl_prob
                return inputs * self.u
            else:
                self.u = self.rng.uniform(0,1, size = inputs.shape) < self.incl_prob
                return inputs * self.u
        else:
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        return grads_wrt_outputs * self.u
